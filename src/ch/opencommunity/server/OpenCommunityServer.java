	...
	public OpenCommunityServer(AppLoader loader){
		...
		*/
		...
		try{ //icons for cms
			...
			String values = "1;Mitglied Stiftungsrat: Präsident\n";
			values += "2;Mitglied Stiftungsrat: Vizepräsident\n";
			values += "3;Mitglied Stiftungsrat: normales Mitglied\n";
			values += "4;Geschäftsführer oder Sekretär\n";
					...
					*/
						...
						if(sectionid != null){
							...
							if(sectionid.equals("home")){
								...
								//html.append("<input type=\"button\" onclick=\"javascript:createProcess(\'ch.opencommunity.process.SendRequestConfirmation\',\'Mode=3\')\" value=\"Verlängerung anfragen\">");
								html.append("<input type=\"button\" onclick=\"javascript:createProcess(\'ch.opencommunity.process.MemberAdSendRenewalRequest\',\'Mode=1\')\" value=\"Verlängerung anfragen per Email\">");
								html.append("<input type=\"button\" onclick=\"javascript:createProcess(\'ch.opencommunity.process.MemberAdSendRenewalRequest\',\'Mode=2\')\" value=\"Verlängerung anfragen per Brief\">");
								html.append("<input type=\"button\" onclick=\"javascript:createProcess(\'ch.opencommunity.process.MemberAdSendRenewalRequest\',\'Mode=3\')\" value=\"Abgelaufene Inserate löschen\">");
								...
								*/
								...
								if(istodelete != null && istodelete.length() > 0){
									...
									if(!reload){
										...
										filterdef.append(" Inkl. zu löschende <input type=\"checkbox\" name=\"IsToDelete\" value=\"1\">");
								...
								*/
								...
								if(!reload){
									...
									filterdef.append(" Gültig von zw. " + getFormManager().getDateField(userSession.getProperty("ValidFrom1"), true, ""));
...
;									result.setData(tba.toHTML(context));
						...
						familyname = familyname.replaceAll("ue", "ü");				
						familyname = familyname.replaceAll("ü", "(ü|u|ue)");
	
						familyname = familyname.replaceAll("ae", "ä");		
						familyname = familyname.replaceAll("ä", "(ä|a|ae)");
	
						familyname = familyname.replaceAll("oe", "ö");		
						familyname = familyname.replaceAll("ö", "(ö|o|oe)");
						
						firstname = firstname.replaceAll("ue", "ü");
						firstname = firstname.replaceAll("ü", "(ü|u|ue)");
	
						firstname = firstname.replaceAll("ae", "ä");		
						firstname = firstname.replaceAll("ä", "(ä|a|ae)");
	
						firstname = firstname.replaceAll("oe", "ö");	
						firstname = firstname.replaceAll("ö", "(ö|o|oe)");
						...
						if(om != null){
							...
							if(id != null){
								if(id.equals(om.getName())){
									...
									result = new ActionResult(Status.OK, "Profil geändert");
									result.setParam("refresh", "currentsection");										
								}
								else{
						...
						//					System.out.println(fieldname + ":" + value);
							...
							//					process.suspend();
												if (userSession.getSuspendedProcess() != null) {
													...
													result.setStatusMsg("Es kann höchstens ein Prozess gleichzeitig unterbrochen sein");
												}
												else {
													userSession.setCurrentProcess(null);
	...
	public String getOrganisationMemberList(ApplicationContext context, String filter, String sectionid, boolean embedded){
		...
		String[] status = {"registriert", "aktiv", "inaktiv", "zu löschen", "zu kontrollieren"};
		...
		html.append("<th class=\"tableheader\">Zul. geändert</th>");
		...
		html.append("<th class=\"tableheader\">Adressänderung pendent</th>");
	...
	public String getMemberAdRequestList(ApplicationContext context, OpenCommunityUserSession usersession, String filter, String sectionid, boolean embedded){
		...
		sql += " INNER JOIN OrganisationMember AS t10 ON t1.OrganisationMemberID=t10.ID"; //Eigentümer des Inserates
		...
		html.append("<th>Nachname</th><th>Vorname</th><th><a href=\"javascript:loadSection(\'requests\',\'sortfield=8')\">Inserateigentümer</a></th>");
	...
	public String getFeedbackList2(ApplicationContext context, OpenCommunityUserSession usersession, String filter, String sectionid){
		...
		html.append("<th><a href=\"javascript:loadSection(\'memberads\',\'sortfield=t1.ValidFrom\')\">Gültig von</a></th>");
		html.append("<th><a href=\"javascript:loadSection(\'memberads\',\'sortfield=t1.ValidUntil\')\">Gültig bis</a></th>");
	...
	public String getDocumentList(){
		...
		StringBuilder html = new StringBuilder();
		html.append("<table>");
		html.append("<tr><th>ID</th><th>Aktivität</th><th>Vorlage</th><th>Betreff</th><th>Datum</th><th>Name Empfänger</th><th>Vorname Empfänger</th><th>Status</th></tr>");
		ObjectCollection results = new ObjectCollection("Results", "*");
	...
	public List<ConfigValue> getSupportedLanguages(){
		...
		languages.add(new ConfigValue("french","french","Französisch"));
		...
		languages.add(new ConfigValue("turkish","turkish","Türkisch"));
		*/
		return languages;
	}
	...
	public String getBaseURL(String path,  HttpServletRequest request){
		...
		if(port==80){
			return "http://" + host;	//appliikation hinter apachce, bessere Lösung finden
		}
		else{
